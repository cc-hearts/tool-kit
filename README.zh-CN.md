...
| **Background Remover** | 图片处理 | 本地 AI 抠图：浏览器端运行分割模型（u2netp）去除背景，棋盘格预览透明效果，支持画笔恢复/擦除微调，导出透明 PNG，图片不上传服务器 |
...
| **Cookie Parser** | 开发辅助 | 将复制来的 Cookie 文本（请求 Cookie、Set-Cookie 响应头或 JSON）解析为结构化 JSON 或表格视图 |
